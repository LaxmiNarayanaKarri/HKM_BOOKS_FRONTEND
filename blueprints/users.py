...
from services.api_client import BackendError, get_file, get_json, post_json
...
def _overview_filters():
    ...
    return {
        "ov_date_from": (request.values.get("ov_date_from") or "").strip(),
        "ov_date": (request.values.get("ov_date") or "").strip(),
...
def _fetch_overview_payload(filters):
    ...
    This is the fix for the original bug: sales_data was being fetched from
    the books service but never merged into page_data, so ov_total_sales /
    ov_sales_by_location always rendered as 0 / {} regardless of what the
    books service actually returned.
    """
    try:
        users_data = get_json(_base_url(), "/api/users/volunteers/data", params=filters)
    except BackendError as exc:
        flash(f"Could not load volunteer assignment data: {exc}", "danger")
        users_data = {
            "volunteers": [],
            "locations": [],
            "assignments": [],
    ...
    try:
        print(_base_url_books(), filters)
        sales_data = get_json(_base_url_books(), "/internal/location-overview", params=filters)
    except BackendError as exc:
        flash(f"Could not load sales data: {exc}", "danger")
        sales_data = {"sales_by_location": {}, "total_sales": 0}
